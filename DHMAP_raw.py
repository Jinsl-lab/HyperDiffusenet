...
class scDHMap_raw(nn.Module):
    def __init__(self, input_dim, encodeLayer=[], decodeLayer=[], batch_size=512,
        ...
        super(scDHMap_raw, self).__init__()
    ...
    def aeForward(self, x):
        ...
        z_sigma_square = self.enc_sigma(h).clamp(min=1e-6, max=20).clone()  # 同上
    ...
    def pretrain_autoencoder(self, X, X_raw, size_factor, lr=0.001, pretrain_iter=200):
        """
        ...
        for epoch in range(pretrain_iter):
            ...
            print('Pretraining epoch {}, Total loss:{:.8f}, reconn loss:{:.8f}, KLD loss:{:.8f}'.format(epoch + 1,
                                                                                                        loss_val,
                                                                                                        loss_reconn_val,
                                                                                                        loss_kld_val))


    def train_model(self, X, X_raw, size_factor, X_pca,  lr=0.001, maxiter=5000, minimum_iter=0,
                    patience=50):
        """
...
#                x_raw_tensor = torch.from_numpy(x_raw_batch).to(self.device)
#                sf_tensor = torch.from_numpy(sf_batch).to(self.device)
#                p_tensor = torch.from_numpy(p_batch).to(self.device)
            ...
            print('Training epoch {}, Total loss:{:.8f}, reconn loss:{:.8f}, t-SNE loss:{:.8f}, KLD loss:{:.8f}'.format(
                epoch + 1, loss_val, loss_reconn_val, loss_tsne_val, loss_kld_val))
