...
class PoissonLoss(nn.Module):
    ...
    def forward(self, x, mean,scale_factor=None):
        """
        计算泊松损失
        参数:
        - mean: 模型预测的事件平均发生率，大小为 [batch_size, ...]
        - x: 实际观测的事件计数，大小与mean相同
        返回:
        - loss: 泊松损失的平均值
        """
        eps=1e-10
        if scale_factor is not None:
            scale_factor = scale_factor[:, None]
            mean = mean * scale_factor
        # 确保mean大于0以避免对数运算中的无效值
        mean = torch.clamp(mean, min=eps)
        loss = mean - x * torch.log(mean)
        # 计算所有样本的损失平均值
        result = torch.mean(torch.sum(loss, dim=1))
        return result
