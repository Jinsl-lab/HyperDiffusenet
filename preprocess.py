...
def pearson_residuals(counts, theta, clipping=True):
    ...
    z = (counts - mu) / (np.sqrt(mu + mu**2/theta)+eps)
...
def normalize(adata, filter_min_counts=True, size_factors=True, normalize_input=True, logtrans_input=True, norm='l2',
              axis=1):
    ...
    original_obsm = adata.obsm.copy()
    original_layers = adata.layers.copy()
    ...
    for col in original_obs.columns:
        if col not in adata.obs.columns:
            adata.obs[col] = original_obs[col]

    for col in original_var.columns:
        if col not in adata.var.columns:
            adata.var[col] = original_var[col]

    # 恢复 uns, obsm 和 layers
    adata.uns.update(original_uns)
    adata.obsm.update(original_obsm)
    adata.layers.update(original_layers)
